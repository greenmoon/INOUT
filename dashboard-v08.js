(function(){
 ...
 let viewMode='io',connectionConfig=null;
 // Presentation-only baselines, bounded and never persisted or published.
 const countSessions=new Map();
 const countKey=p=>p.id+'/'+p.s;
 function observeCount(p){
  const key=countKey(p);let entry=countSessions.get(key);
  if(!entry){if(countSessions.size>=192)countSessions.delete(countSessions.keys().next().value);entry={mode:'acc',base:null,last:null};countSessions.set(key,entry);}
  if(entry.last&&(p.i<entry.last.i||p.o<entry.last.o||p.f<entry.last.f)&&entry.base)entry.base.invalid=true;
  entry.last={i:p.i,o:p.o,f:p.f};return entry;
 }
 function countPresentation(r){
  const entry=r?countSessions.get(countKey(r.p)):null,relative=entry&&entry.mode==='rst',base=relative?entry.base:null;
  return {relative,valid:!relative||!!base&&!base.invalid,
   i:r?relative?base&&!base.invalid?r.p.i-base.i:null:r.p.i:null,
   o:r?relative?base&&!base.invalid?r.p.o-base.o:null:r.p.o:null};
 }
 ...
 function controls(){
  $('connect').textContent=viewMode==='diag'?'重新連接':'連接';
  $('connect').disabled=viewMode==='diag'?!connectionConfig||!window.mqtt||invalidLink:wanted;
  $('disconnect').disabled=!wanted;
 ...
 function render(){
  ...
  const counts=countPresentation(r);
  $('inValue').textContent=counts.i===null?'—':String(counts.i);$('outValue').textContent=counts.o===null?'—':String(counts.o);
  $('accButton').setAttribute('aria-pressed',String(!counts.relative));$('rstButton').setAttribute('aria-pressed',String(!!counts.relative));
  $('rstButton').disabled=!view.fresh;
  $('countModeLabel').textContent=counts.relative?(counts.valid?'RST · 本次計數':'RST · 基準失效，請重新 RST'):'ACC · 累積計數';
  $('countModeLabel').dataset.invalid=String(!counts.valid);
 ...
 function stop(text='已斷開',kind='idle'){
  autoPending=false;
  if(connectionConfig)connectionConfig={...connectionConfig,password:''};
  wanted=false;active=null;closeTransport();$('password').value='';state(text,kind);controls();render();
 }
 function start(config,reset){
  if(connectionConfig&&(connectionConfig.url!==config.url||connectionConfig.topic!==config.topic))countSessions.clear();
  ...
  try{
   ...
   c.on('message',(topic,payload,packet={})=>{
    ...
    if(result.kind==='valid'){observeCount(store.latest.p);devices();notice('ACC 原始累積／RST 本次增量；目前選定 '+selected+'。');}
 ...
 function connect(){
  ...
  if(viewMode==='diag'){
   if(window.mqtt&&connectionConfig){
    log('診斷：使用目前設定重新連接');
    start({...connectionConfig},false);
   }
   return;
  }
 ...
 $('configForm').onsubmit=event=>{event.preventDefault();if(viewMode==='rd')connect();};
 $('disconnect').onclick=()=>{stop();notice('已手動斷開；不會自動重新連線。');log('使用者斷開');};
 $('device').onchange=()=>{closeRdEntry();selected=$('device').value;remember();if(viewMode==='rd')mode(false,true);else render();};
 $('remember').onchange=remember;
 function mode(rd,diagnostic=false){
  viewMode=diagnostic?'diag':rd?'rd':'io';
  const details=viewMode!=='io',full=viewMode==='rd';
  $('app').dataset.mode=viewMode;$('rd').hidden=!details;$('io').hidden=details;
  $('rdButton').setAttribute('aria-pressed',String(details));$('ioButton').setAttribute('aria-pressed',String(!details));
  $('settings').hidden=!full;if(!full)$('settings').open=false;
  for(const id of ['url','topic','stale','username','password','remember','applyConnect'])$(id).disabled=!full;
  $('diagHint').hidden=!diagnostic;$('fullRdButton').hidden=!diagnostic;
  $('diagnosticTitle').textContent=diagnostic?'連線診斷 · 免碼':'接收診斷 · RD';
  controls();render();
 }
 ...
 let rdExpected=null,rdSlot=null;
 function clearRdEntry(){
  rdExpected=null;rdSlot=null;$('rdSnapshot').textContent='';$('rdEntrySubmit').disabled=false;
 ...
 $('diagnosticsEntry').onclick=()=>{closeRdEntry();mode(false,true);$('fullRdButton').focus();};
 $('fullRdButton').onclick=()=>$('rdButton').onclick();
 $('rdButton').onclick=()=>{
  ...
  if(record&&view.fresh){
   rdSlot=record.p.s;
   const inText=String(record.p.i),outText=String(record.p.o);
   rdExpected=inText+outText.split('').reverse().join('');
   $('rdSnapshot').textContent=selected+' · S'+record.p.s+' · ACC IN '+inText+' / OUT '+outText+'（已固定；不使用 RST）';
  }else{
   $('rdSnapshot').textContent='尚無此裝置的新鮮有效 ACC 數值（離線／舊值不可解鎖，— 不是 0）。可先按「連線診斷」查看狀態及重新連接。';
   $('rdEntrySubmit').disabled=true;
  }
  rdDialog.showModal();(record&&view.fresh?rdInput:$('diagnosticsEntry')).focus();
 };
 ...
 $('rdEntryForm').onsubmit=event=>{
  ...
  const currentView=store.view(selected,now(),timeout,online&&subscribed&&!document.hidden);
  if(!currentView.fresh||currentView.record.p.s!==rdSlot){$('rdEntryError').textContent='資料已過期或 slot 已變更，請取消後重新取得 ACC 快照。';return;}
 ...
 $('accButton').onclick=()=>{
  closeRdEntry();const r=store.view(selected,now(),timeout,online&&subscribed&&!document.hidden).record;
  if(r){const entry=countSessions.get(countKey(r.p));if(entry)entry.mode='acc';}
  if(viewMode==='rd')mode(false);else render();
 };
 $('rstButton').onclick=()=>{
  const view=store.view(selected,now(),timeout,online&&subscribed&&!document.hidden);
  if(!view.fresh)return;
  closeRdEntry();const p=view.record.p,entry=observeCount(p);
  entry.mode='rst';entry.base={i:p.i,o:p.o,invalid:false};
  log('本畫面 RST：'+p.id+' / S'+p.s+'；Radar 未重設。');
  if(viewMode==='rd')mode(false);else render();
 };
